...
public class GameCommander {
	...
	public void printDiffTime(long pretime, int line) {
		long curtime, difftime, maxdifftime=50;
...
//					+ new Exception().getStackTrace()[0].getLineNumber());
	...
	public void onFrame() {
		...
		if (MyBotModule.Broodwar.isPaused() || MyBotModule.Broodwar.self() == null
				|| MyBotModule.Broodwar.self().isDefeated() || MyBotModule.Broodwar.self().leftGame()
				|| MyBotModule.Broodwar.enemy() == null || MyBotModule.Broodwar.enemy().isDefeated()
		...
		try {
			...
			printDiffTime(pretime, new Exception().getStackTrace()[0].getLineNumber());
	...
	public void onUnitCreate(Unit unit) {
		...
		if (unit.getType() == UnitType.Terran_Command_Center
				&& unit.getPlayer() == InformationManager.Instance().selfPlayer) {
...
//				unit.lift();
				...
				// myUnit.isCompleted())
				// {
				// unit.setRallyPoint(new Position( (unit.getX()*7 + myUnit.getX())/8,
				// (unit.getY()*7 + myUnit.getY())/8));
				// }
				// }
			} else {
				for (Unit myUnit : MyBotModule.Broodwar.self().getUnits()) {
