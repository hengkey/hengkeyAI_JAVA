//# Prebot (피뿌리는컴파일러 / 알고리즘 경진대회 2017)s
...
public class StrategyManager {
	...
	public void update() {
		...
		if ((MyBotModule.Broodwar.getFrameCount() < 13000 && MyBotModule.Broodwar.getFrameCount() % 5 == 0)
				|| (MyBotModule.Broodwar.getFrameCount() >= 13000 && MyBotModule.Broodwar.getFrameCount() % 23 == 0)) { // Analyze
																														// 와
																														// 동일하게
	...
	private int GetCurrentTotBlocked(UnitType checkunit) {
		...
		// if(unit.getTrainingQueue().size() > 0 && unit.getTrainingQueue().get(0) ==
		// checkunit){
		// cnt ++;
		// }
		// }
	...
	public void executeWorkerTraining() {
		...
		if (EXOK == false) {
			if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) == 2) {
				...
				if (checkCC != null) {
					BaseLocation temp = InformationManager.Instance()
						...
						if (!tempbuildQueue.isEmpty()) {
							...
							while (true) {
								...
								if (checkItem.metaType.isUnit()
										&& checkItem.metaType.getUnitType() == UnitType.Terran_SCV) {
		...
		if (MyBotModule.Broodwar.self().minerals() >= 50) {
			int maxworkerCount = tot_mineral_self * 2
					+ 8 * MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center);
			int workerCount = MyBotModule.Broodwar.self().allUnitCount(InformationManager.Instance().getWorkerType()); // workerCount
																														// =
																														// 현재
																														// 일꾼
																														// 수
					...
					if (unit.getType().isResourceDepot() && unit.isCompleted() && unit.isTraining() == false) {
						...
						if (!tempbuildQueue.isEmpty()) {
							...
							while (true) {
								...
								// if(checkItem.metaType.isUnit() &&
								// checkItem.metaType.getUnitType().isAddon()){
								// return;
								// }
						...
						if (checkItem == null) {
							...
							if (checkItem.metaType.getUnitType() == UnitType.Terran_Comsat_Station) {
								...
								if (workerCount < 4) {
									...
									int checkgas = checkItem.metaType.getUnitType().gasPrice()
											- MyBotModule.Broodwar.self().gas();
									if (checkgas < 0) {
										checkgas = 0;
									}
									if (MyBotModule.Broodwar.self()
											.minerals() > checkItem.metaType.getUnitType().mineralPrice() + 50
													- checkgas) {
										BuildManager.Instance().buildQueue.queueAsHighestPriority(
												new MetaType(InformationManager.Instance().getWorkerType()), false);
									}
	...
	public void executeSupplyManagement() {
		...
		if (MyBotModule.Broodwar.self().supplyTotal() < 400) {
			...
			} else { // if((MyBotModule.Broodwar.getFrameCount()>=6000 &&
						// MyBotModule.Broodwar.getFrameCount()<10000) || (Faccnt > 3 && CCcnt == 2)){
				supplyMargin = 11 + 4 * Faccnt + facFullOperating * 2;
			}

			// currentSupplyShortage 를 계산한다
			int currentSupplyShortage = MyBotModule.Broodwar.self().supplyUsed() + supplyMargin + 1
			...
			if (currentSupplyShortage > 0) {
				...
				if (currentSupplyShortage > onBuildingSupplyCount) {
					...
					BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Supply_Depot,
							BuildOrderItem.SeedPositionStrategy.NextSupplePoint, false);
					//
					//// boolean isToEnqueue = true;
					//// if (!BuildManager.Instance().buildQueue.isEmpty()) {
					//// BuildOrderItem currentItem =
					// BuildManager.Instance().buildQueue.getHighestPriorityItem();
					//// if (currentItem.metaType.isUnit() && currentItem.metaType.getUnitType() ==
					// InformationManager.Instance().getBasicSupplyProviderUnitType())
					//// {
					//// isToEnqueue = false;
					//// }
					//// }
					// if(InformationManager.Instance().getMainBaseSuppleLimit() <=
					// MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Supply_Depot)){
					//// MyBotModule.Broodwar.printf("currentSupplyShortage: " +
					// currentSupplyShortage);
					//// MyBotModule.Broodwar.printf("onBuildingSupplyCount: " +
					// onBuildingSupplyCount);
					//// MyBotModule.Broodwar.printf("supplyMargin: " + supplyMargin);
					//// MyBotModule.Broodwar.printf("supplyTotal: " +
					// MyBotModule.Broodwar.self().supplyTotal());
					//
					// }else{
					//// MyBotModule.Broodwar.printf("currentSupplyShortage: " +
					...
					//// MyBotModule.Broodwar.printf("supplyTotal: " +
					// MyBotModule.Broodwar.self().supplyTotal());
					// BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Supply_Depot,BuildOrderItem.SeedPositionStrategy.NextSupplePoint,
					// true);
					// }
				}
			}
	...
	public void executeAddBuildingInit() {
		...
		if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Factory, null)
				+ MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Factory) + ConstructionManager.Instance()
						.getConstructionQueueItemCount(UnitType.Terran_Factory, null) < InitFaccnt) {
	...
	public void executeSustainUnits() {
			...
			if (unit.getType() == UnitType.Terran_Command_Center && unit.isCompleted()) {
				...
				if (isInitialBuildOrderFinished == false && geyserAround != null) {
					if (InformationManager.Instance().getMyfirstGas() != null) {
						if (geyserAround.getPosition()
								.equals(InformationManager.Instance().getMyfirstGas().getPosition())) {
							continue;
						}
					}
				}
				...
				if (refineryAlreadyBuilt == false) {
					TilePosition findGeyser = ConstructionPlaceFinder.Instance()
							.getRefineryPositionNear(unit.getTilePosition());
					if (findGeyser != null) {
				...
				if (unit.getType() == UnitType.Terran_Command_Center && unit.isCompleted() && unit.canBuildAddon()
					...
					if (MyBotModule.Broodwar.self().minerals() > 50 && MyBotModule.Broodwar.self().gas() > 50) {
						if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Comsat_Station, null)
							...
							// 종종 초반에 command center가 1개인데 comsat을 2개 큐에 넣는 현상이 있어서 방어코드로 넣음.이럴경우 다음 빌드큐를
							// 수행하지 못하고 걸린다.
							if (MyBotModule.Broodwar.self()
		...
		// barrack start
		if (isInitialBuildOrderFinished == true && (barrackcnt == 0
				|| (barrackcnt == 1 && barrackUnit.getHitPoints() < UnitType.Terran_Barracks.maxHitPoints() / 2))) {
			if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Barracks) == 0 && ConstructionManager
					.Instance().getConstructionQueueItemCount(UnitType.Terran_Barracks, null) == 0) {
		...
		// engineering start
		if (isInitialBuildOrderFinished == true
				&& (engineeringcnt == 0 || (engineeringcnt == 1
						&& engineeringUnit.getHitPoints() < UnitType.Terran_Engineering_Bay.maxHitPoints() / 2))
		...
		if ((RespondToStrategy.Instance().need_vessel == true && CC >= 1) || CC >= 2) {
			...
			if (starComplete && scienceComplete && controltower) {
				if (MyBotModule.Broodwar.self()
						.allUnitCount(UnitType.Terran_Science_Vessel) < RespondToStrategy.Instance().max_vessel) {
					if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Science_Vessel, null)
							+ ConstructionManager.Instance()
									.getConstructionQueueItemCount(UnitType.Terran_Science_Vessel, null) == 0) {
		...
		// battlecruiser start
		// System.out.println("need_battlecruiser: " +
		// RespondToStrategy.Instance().need_battlecruiser);
		// System.out.println("CC: " + CC);
		// System.out.println("max_battlecruiser: " +
		// RespondToStrategy.Instance().max_battlecruiser);
		// System.out.println("battlecnt: " + battlecnt);

		// if(RespondToStrategy.Instance().need_battlecruiser == true && CC>=3 &&
		// RespondToStrategy.Instance().max_battlecruiser > battlecnt){
		//
		// nomorewraithcnt = 100;
		// if(MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Starport) < 4){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Starport)
		// == 0
		// &&
		// ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Starport,
		// null) < 4) {
		...
		// if(MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Control_Tower) <
		// 4){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Control_Tower)
		// == 0
		// &&
		// ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Control_Tower,
		// null) < 4) {
		// if(Config.BroodwarDebugYN){
		// MyBotModule.Broodwar.printf("make starport for battle");
		// }
		...
		// }
		// if(science == false && starComplete){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Science_Facility)
		// == 0
		// &&
		// ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Science_Facility,
		// null) == 0) {
		// if(Config.BroodwarDebugYN){
		...
		// }
		// if(scienceComplete){
		// if(MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Physics_Lab) <
		// 1){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Physics_Lab,
		// null)
		// +
		// ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Physics_Lab,
		// null) == 0){
		// BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Physics_Lab,
		// false);
		// }
		// }else if(physiclab){
		// if(MyBotModule.Broodwar.self().hasResearched(TechType.Yamato_Gun) == false &&
		// MyBotModule.Broodwar.self().isResearching(TechType.Yamato_Gun) ==false){
		// if(BuildManager.Instance().buildQueue.getItemCount(TechType.Yamato_Gun) ==
		// 0){
		...
		// if(MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Battlecruiser) <
		// RespondToStrategy.Instance().max_battlecruiser){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Battlecruiser,
		// null) == 0) {
		// BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Battlecruiser,
		// false);
		// }
		// }
		// }
		...
		if (RespondToStrategy.Instance().max_wraith > wraithcnt
				&& nomorewraithcnt <= RespondToStrategy.Instance().max_wraith) {
		...
		// }
		// if(armory == false){
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Armory) ==
		// 0
		// &&
		// ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Armory,
		// null) == 0) {
		// if(Config.BroodwarDebugYN){
		...
		// }
		// }
		// if(starComplete && starportUnit.getAddon() != null &&
		// starportUnit.getAddon().isCompleted() == true){
		// if(starportUnit.isTraining() == false){// && valkyriecnt <
		// needwraithvalkyriecnt){ //TODO
		// if(BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Valkyrie,
		// null) == 0) {
		// if(Config.BroodwarDebugYN){
	...
	public void executeAddFactory() {
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
			...
			if (unit.getType() == UnitType.Terran_Factory && unit.isCompleted()) {
				...
				if (BuildManager.Instance().isBuildableTile(unit.getTilePosition().getX() + 4,
						unit.getTilePosition().getY() + 1) == false
						|| BuildManager.Instance().isBuildableTile(unit.getTilePosition().getX() + 5,
								unit.getTilePosition().getY() + 1) == false
						|| BuildManager.Instance().isBuildableTile(unit.getTilePosition().getX() + 4,
		...
		if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Factory, null) == 0) {
			if (Faccnt == 0) {
				TilePosition nearTilePosition = new TilePosition(BlockingEntrance.Instance().factoryX,
						BlockingEntrance.Instance().factoryY);
				BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Factory, nearTilePosition,
						true, true);// 강제건설
				// BuildManager.Instance().buildQueue.queueAsHighestPriority(UnitType.Terran_Factory,BuildOrderItem.SeedPositionStrategy.MainBaseLocation,
				// true);
			} else if (Faccnt < maxFaccnt) {
				if (MyBotModule.Broodwar.self().minerals() > 200 + additonalmin
						&& MyBotModule.Broodwar.self().gas() > 100 + additonalgas) {
					if (Faccnt == 2) {
						if (getFacUnits() > 24 && facFullOperating) {
							TilePosition nearTilePosition = new TilePosition(BlockingEntrance.Instance().factoryX,
									BlockingEntrance.Instance().factoryY + 3);
							BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,
									nearTilePosition, false, true);// 강제건설
							// BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,BuildOrderItem.SeedPositionStrategy.MainBaseLocation,
							// false);
						}
					} else if (Faccnt == 3) {
						TilePosition nearTilePosition = new TilePosition(BlockingEntrance.Instance().factoryX + 7,
								BlockingEntrance.Instance().factoryY + 3);
						BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,
								nearTilePosition, false, true);// 강제건설
					} else if (Faccnt == 4) {
						TilePosition nearTilePosition = new TilePosition(BlockingEntrance.Instance().factoryX,
								BlockingEntrance.Instance().factoryY + 6);
						BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,
								nearTilePosition, false, true);// 강제건설
					} else if (Faccnt == 5) {
						TilePosition nearTilePosition = new TilePosition(BlockingEntrance.Instance().factoryX + 7,
						...
						BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,
								BuildOrderItem.SeedPositionStrategy.MainBaseLocation, false);
						// BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Factory,
						// BuildOrderItem.SeedPositionStrategy.LastBuilingPoint, false);
					}
				}
			}
		}
		...
		if (MachineShopcnt < FaccntForMachineShop) {
			for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
				if (unit == null)
					continue;
				if (unit.getType() == UnitType.Terran_Factory && unit.isCompleted() && unit.isCompleted()
						&& unit.getAddon() == null && unit.canBuildAddon()) {
					int addition = 3;
					if (MyBotModule.Broodwar.self().gas() > 300) {

						int tot_vulture = GetCurrentTotBlocked(UnitType.Terran_Vulture);
						int tot_tank = GetCurrentTotBlocked(UnitType.Terran_Siege_Tank_Tank_Mode)
								+ GetCurrentTotBlocked(UnitType.Terran_Siege_Tank_Siege_Mode);
						int tot_goliath = GetCurrentTotBlocked(UnitType.Terran_Goliath);
						UnitType selected = chooseunit(vultureratio, tankratio, goliathratio, wgt, tot_vulture,
								tot_tank, tot_goliath);
					...
					if (MachineShopcnt + addition < FaccntForMachineShop) {
						if (MyBotModule.Broodwar.self().minerals() > 50 && MyBotModule.Broodwar.self().gas() > 50) {
							if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Machine_Shop, null) == 0
									&& ConstructionManager.Instance()
											.getConstructionQueueItemCount(UnitType.Terran_Machine_Shop, null) == 0) {
	...
	public void executeCombatUnitTrainingBlocked() {
		...
		if (MyBotModule.Broodwar.self().supplyTotal() - MyBotModule.Broodwar.self().supplyUsed() < 4) {
			// System.out.println("executeCombatUnitTrainingBlocked "+ new
		...
		if (!tempbuildQueue.isEmpty()) {
			...
			while (true) {
				...
				if (currentItem.metaType.isUnit() && currentItem.metaType.getUnitType() == UnitType.Terran_Goliath) {
					// System.out.println("executeCombatUnitTrainingBlocked " +
					// currentItem.metaType.getUnitType() + " "
					// + new Exception().getStackTrace()[0].getLineNumber());
					goliathInTheQueue = true;
				}
				if (currentItem.metaType.isUnit()
						&& currentItem.metaType.getUnitType() == UnitType.Terran_Siege_Tank_Tank_Mode) {
					// System.out.println("executeCombatUnitTrainingBlocked " +
					// currentItem.metaType.getUnitType() + " "
					// + new Exception().getStackTrace()[0].getLineNumber());
					tankInTheQueue = true;
			...
			// System.out.println("executeCombatUnitTrainingBlocked isEmpty "+ new
			// Exception().getStackTrace()[0].getLineNumber());
			return;
		}
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
			...
			if (unit.getType() == UnitType.Terran_Factory && unit.isCompleted() && unit.isTraining() == false) {
				...
				if (currentItem.metaType.isUnit() && currentItem.metaType.getUnitType() == UnitType.Terran_Machine_Shop
						&& unit.getAddon() == null) {
					continue;
				}
				if (currentItem.metaType.isUnit()
					...
					if (selected == UnitType.Terran_Siege_Tank_Tank_Mode && tankInTheQueue == false) {
						if (unit.getAddon() != null && unit.getAddon().isCompleted() == true) {
							if (currentItem.metaType.mineralPrice() + minNeed < MyBotModule.Broodwar.self().minerals()
									&& currentItem.metaType.gasPrice() + selected.gasPrice() < MyBotModule.Broodwar
											.self().gas()
									&& MyBotModule.Broodwar.self().supplyUsed() <= 392) {
								BuildManager.Instance().buildQueue.queueAsHighestPriority(selected,
										BuildOrderItem.SeedPositionStrategy.MainBaseLocation, false);
				...
				if (eventually_vulture) {
					...
					if (currentItem.metaType.mineralPrice() + minNeed < MyBotModule.Broodwar.self().minerals()
							&& MyBotModule.Broodwar.self().supplyUsed() <= 392) {
						if ((unit.isConstructing() == true) || ConstructionManager.Instance()
								.getConstructionQueueItemCount(UnitType.Terran_Machine_Shop, null) != 0) {
							continue;
						}
	...
	public void executeCombatUnitTraining() {
		...
		int tot_tank = GetCurrentTot(UnitType.Terran_Siege_Tank_Tank_Mode)
				+ GetCurrentTot(UnitType.Terran_Siege_Tank_Siege_Mode);
		...
		if (currentinbuildqueuecnt == 0) {
			...
			if (selected.mineralPrice() < MyBotModule.Broodwar.self().minerals()
					&& selected.gasPrice() < MyBotModule.Broodwar.self().gas()
					&& MyBotModule.Broodwar.self().supplyUsed() <= 392) {
				BuildManager.Instance().buildQueue.queueAsLowestPriority(selected,
						BuildOrderItem.SeedPositionStrategy.MainBaseLocation, false);
			}
		}
	}
	...
	public int getTotKilledCombatUnits() {
		...
		if (InformationManager.Instance().enemyRace == Race.Terran) {
			...
			res = totzerling + +totsunken * 2 + tothydra * 2 + totmutal * 4 + totoverload * 3 + totlurker * 5
					+ totguardian * 8 + totultra * 10;
	...
	public boolean enemyExspansioning() {
		// int cnt =
		// MyBotModule.Broodwar.enemy().incompleteUnitCount(InformationManager.Instance().getBasicResourceDepotBuildingType(InformationManager.Instance().enemyRace));
		List<UnitInfo> enemyCC = null;
		enemyCC = InformationManager.Instance().getEnemyUnits(InformationManager.Instance()
				.getBasicResourceDepotBuildingType(InformationManager.Instance().enemyRace));
	...
	public void executeCombat() {
		...
		selfbasecnt = InformationManager.Instance()
				.getOccupiedBaseLocationsCnt(InformationManager.Instance().selfPlayer);
		enemybasecnt = InformationManager.Instance()
				.getOccupiedBaseLocationsCnt(InformationManager.Instance().enemyPlayer);
		...
		int totworkerkilled = MyBotModule.Broodwar.self().killedUnitCount(
				InformationManager.Instance().getWorkerType(InformationManager.Instance().enemyRace)) * 2;
		...
		if (MyBotModule.Broodwar.getFrameCount() < 20000) { // 약 시작 ~ 15분까지
			unitPoint += (totworkerkilled - totworkerdead)
					* (-MyBotModule.Broodwar.getFrameCount() / 40000.0 * 3.0 + 3.0);
		...
		// 종족별 예외 상황
		if (InformationManager.Instance().enemyRace == Race.Terran) {
			if (myunitPoint > 80 && killedcombatunit - deadcombatunit > myunitPoint / 4) {// 죽인수 - 죽은수 가 현재 내 유닛의 일정 비율이
																							// 넘으면 가산점
				unitPoint += 20;
			}
		}
		if (InformationManager.Instance().enemyRace == Race.Protoss) {
			if (myunitPoint > 80 && killedcombatunit - deadcombatunit > myunitPoint / 3) {// 죽인수 - 죽은수 가 현재 내 유닛의 일정 비율이
																							// 넘으면 가산점
				unitPoint += 20;
			}

			unitPoint += InformationManager.Instance().getNumUnits(UnitType.Protoss_Photon_Cannon,
		...
		if (InformationManager.Instance().enemyRace == Race.Zerg) {
			...
			unitPoint += InformationManager.Instance().getNumUnits(UnitType.Zerg_Sunken_Colony,
					InformationManager.Instance().enemyPlayer) * 4;
			// triple hatchery
		}
		...
		if (InformationManager.Instance().enemyRace == Race.Terran) {
			...
			if (unitPoint > 10 && MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)
					+ MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 4
							+ plus) {
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
					...
					&& MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)
							+ MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 5
									+ plus) {
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
			} else if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)
					+ MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 6
							+ plus) {
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
			}
			...
			// 공통 예외 상황
			if ((myunitPoint > 170 || MyBotModule.Broodwar.self().supplyUsed() > 392)
					&& CombatManager.Instance().getCombatStrategy() != CombatStrategy.ATTACK_ENEMY) {// 팩토리 유닛 130 이상 또는
																										// 서플 196 이상
																										// 사용시(스타 내부에서는
																										// 2배)
			...
			if (totPoint > 120 && CombatManager.Instance().getCombatStrategy() != CombatStrategy.ATTACK_ENEMY
					&& myunitPoint > 80) {// 팩토리 유닛이 30마리(즉 스타 인구수 200 일때)

				if (InformationManager.Instance().enemyRace == Race.Zerg && InformationManager.Instance()
						.getNumUnits(UnitType.Zerg_Mutalisk, InformationManager.Instance().enemyPlayer) > 6) {
					if (InformationManager.Instance().getNumUnits(UnitType.Zerg_Mutalisk,
							InformationManager.Instance().enemyPlayer) < MyBotModule.Broodwar.self()
			...
			if ((LastStrategyBasic == StrategyManager.Strategys.protossBasic_DoublePhoto
					|| CurrentStrategyBasic == StrategyManager.Strategys.protossBasic_DoublePhoto)
					&& CombatManager.Instance().getCombatStrategy() != CombatStrategy.ATTACK_ENEMY
					&& MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Siege_Tank_Tank_Mode)
							+ MyBotModule.Broodwar.self()
									.completedUnitCount(UnitType.Terran_Siege_Tank_Siege_Mode) >= 1) {
				CombatManager.Instance().setCombatStrategy(CombatStrategy.ATTACK_ENEMY);
				CombatStartCase = 5;
			...
			if (CombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY) {
				...
				if (CombatStartCase == 5) {
					...
					if (CombatTime < MyBotModule.Broodwar.getFrameCount()
							&& ((myunitPoint < 20 && unitPoint < 20) || unitPoint < -10)) {
					...
					if (InformationManager.Instance().getNumUnits(UnitType.Protoss_Zealot,
				...
				if (InformationManager.Instance().enemyRace == Race.Protoss) {
					if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Science_Vessel) == 0
							&& MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Comsat_Station) == 0) {
						...
						if (dark) {
							for (Unit myunit : MyBotModule.Broodwar.self().getUnits()) {
								if (myunit.getType() == UnitType.Terran_Comsat_Station && myunit.isCompleted()
										&& myunit.getEnergy() > 50) {
									energy = true;
								}
							}
				...
				if (CombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY && expansionPoint >= 0
						&& myunitPoint > 120 && unitPoint + totPoint * 0.1 > 55) {
					if (InformationManager.Instance().enemyRace == Race.Protoss
							&& InformationManager.Instance().getNumUnits(UnitType.Protoss_Photon_Cannon,
					...
					if (InformationManager.Instance().enemyRace == Race.Zerg && InformationManager.Instance()
							.getNumUnits(UnitType.Zerg_Sunken_Colony, InformationManager.Instance().enemyPlayer) <= 3) {
						CombatManager.Instance().setDetailStrategy(CombatStrategyDetail.ATTACK_NO_MERCY, 500 * 24);
					}

				}

				if (CombatManager.Instance().getCombatStrategy() == CombatStrategy.DEFENCE_CHOKEPOINT
						|| (CombatManager.Instance().getDetailStrategyFrame(CombatStrategyDetail.ATTACK_NO_MERCY) > 0
...
//		MyBotModule.Broodwar.sendText("UnitPoint=" + UnitPoint + ", getFacUnits()=" + getFacUnits());
		...
		// Fac Unit 30 마리 이상, 일정 이상의 자원 2단계
			...
			if ((MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) >= 2
					&& getFacUnits() > 140)
					|| (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) >= 3
							&& getFacUnits() > 80)) {
	...
	public void executeResearch() {
		...
		if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center) < 2 && currentResearched >= 2
				&& getFacUnits() < 32
				&& !(MyBotModule.Broodwar.self().minerals() > 300 && MyBotModule.Broodwar.self().gas() > 250))
			return;
		...
		// MetaType vsProtoss[] = new MetaType[]{vultureMine, vultureSpeed,
		// TankSiegeMode, GoliathRange};
		...
		MetaType vsProtossBasic_DoublePhoto[] = new MetaType[] { TankSiegeMode, vultureSpeed, vultureMine,
				GoliathRange };
		...
		if (InformationManager.Instance().enemyRace == Race.Protoss) {
			...
			// StrategyManager.StrategysException.protossException_PhotonRush
			// || (CurrentStrategyException == StrategyManager.StrategysException.Init
			// && LastStrategyException ==
			// StrategyManager.StrategysException.protossException_PhotonRush)){
			...
			air = false;
			for (Unit unit : MyBotModule.Broodwar.enemy().getUnits()) {
				if (unit.getType() == UnitType.Protoss_Stargate || unit.getType() == UnitType.Protoss_Arbiter
						|| unit.getType() == UnitType.Protoss_Carrier || unit.getType() == UnitType.Protoss_Corsair
			...
			for (Unit unit : MyBotModule.Broodwar.enemy().getUnits()) {
				if (unit.getType() == UnitType.Terran_Starport || unit.getType() == UnitType.Terran_Science_Facility
						...
						|| unit.getType() == UnitType.Terran_Science_Vessel || unit.getType() == UnitType.Terran_Wraith
			...
			for (Unit unit : MyBotModule.Broodwar.enemy().getUnits()) {
				if (unit.getType() == UnitType.Zerg_Mutalisk || unit.getType() == UnitType.Zerg_Lair
						|| unit.getType() == UnitType.Zerg_Spire || unit.getType() == UnitType.Zerg_Scourge
						|| unit.getType() == UnitType.Zerg_Guardian || unit.getType() == UnitType.Zerg_Devourer) {
					air = true;
				}
			}
				...
				for (int i = 0; i < 4; i++) {
					if (Currentbool[i] == true) {
						...
						if (i == 3 && Current[3].getUpgradeType() == UpgradeType.Charon_Boosters) {
							if (!air && !(MyBotModule.Broodwar.self().minerals() > 200
									&& MyBotModule.Broodwar.self().gas() > 150)) {
								continue;
							}
						}
	...
	public int getValidMineralsForExspansionNearDepot(Unit depot) {
		...
		for (Unit unit : MyBotModule.Broodwar.getAllUnits()) {
			if ((unit.getType() == UnitType.Resource_Mineral_Field) && unit.getDistance(depot) < 450
					&& unit.getResources() > 200) {
				mineralsNearDepot++;
			}
		}
	...
	public void makeTurretNearbyDepot() {
		if (MyBotModule.Broodwar.self().allUnitCount(UnitType.Terran_Command_Center) >= 2) {
			if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Engineering_Bay) > 0) {
				for (Unit depotUnit : MyBotModule.Broodwar.self().getUnits()) {
					if (depotUnit == null)
						continue;

					// 멀티이후 Command_Center 근처에 일정양의 미사일터렛을 건설한다.
					int radiusValue = 8 * 32;
					if (depotUnit.getType() == UnitType.Terran_Command_Center && depotUnit.isCompleted()) {
						int build_turret_cnt = 0;
						List<Unit> turretInRegion = MyBotModule.Broodwar.getUnitsInRadius(depotUnit.getPosition(),
								radiusValue);
						build_turret_cnt = 0;
						...
						// 멀티이후 Command_Center 반지름 radiusValue안에 maxTurret개 이하로 turret이 있는경우.
						int maxTurret = 2;
						if (build_turret_cnt < maxTurret) {
							// System.out.println("executeExpansion=(" + depotUnit.getTilePosition().getX()
							// + ","
							// + depotUnit.getTilePosition().getY() + ") "
							// + new Exception().getStackTrace()[0].getLineNumber());

							for (Unit mineralUnit : MyBotModule.Broodwar.getAllUnits()) {
								// depotUnit 근처 mineral을 찾기위해
								if ((mineralUnit.getType() == UnitType.Resource_Mineral_Field)
										&& mineralUnit.getDistance(depotUnit.getPosition()) < 320) {

									int tileRange = 10, shiftValue = 2;
									...
									if (BuildManager.Instance().buildQueue.getItemCountNear(
										...
										TilePosition nearTilePosition = new TilePosition(
												mineralUnit.getTilePosition().getX() + shiftValue,
									...
									if (BuildManager.Instance().buildQueue.getItemCountNear(
										...
										TilePosition nearTilePosition = new TilePosition(
												...
												mineralUnit.getTilePosition().getY() - shiftValue);
	...
	public void executeExpansion() {
		...
		// Command Center 근처에 일정양의 turret을 건설 함.
		makeTurretNearbyDepot();

		if (MyBotModule.Broodwar.self().incompleteUnitCount(UnitType.Terran_Command_Center) > 0) {
			return;
		}
		if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null) + ConstructionManager
				.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center, null) != 0) {
			return;
		}
...
//				}
		...
		if (RealCCcnt == 1) {// TODO 이거 손봐야된다... 만약 위로 띄어서 해야한다면?? 본진에 지어진거 카운트 안되는 상황에서 앞마당에 지어버리겟네
			...
			if ((MyBotModule.Broodwar.self().minerals() > 200 && getFacUnits() > 40 && UnitPoint > 15)
					|| (getFacUnits() > 60 && Attackpoint > 60 && ExpansionPoint > 0)) {
		...
		if (RealCCcnt >= 2) {
			...
			// 공격시 돈 250 넘으면 멀티하기
			if (CombatManager.Instance().getCombatStrategy() == CombatStrategy.ATTACK_ENEMY
					&& MyBotModule.Broodwar.self().minerals() > 250) {
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center,
								null) == 0) {
					BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Terran_Command_Center,
			...
			// 200 넘고 유리하면
			if (MyBotModule.Broodwar.self().minerals() > 200 && getFacUnits() > 80 && Attackpoint > 40
					&& ExpansionPoint >= 0) {
				if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
						+ ConstructionManager.Instance().getConstructionQueueItemCount(UnitType.Terran_Command_Center,
								null) == 0) {
			...
			if (MyBotModule.Broodwar.getFrameCount() > 14000) {
				...
				if (getFacUnits() > 80 && MyBotModule.Broodwar.self().deadUnitCount()
						- MyBotModule.Broodwar.self().deadUnitCount(UnitType.Terran_Vulture) < 12) {
					if (BuildManager.Instance().buildQueue.getItemCount(UnitType.Terran_Command_Center, null)
							+ ConstructionManager.Instance()
...
//				}
	...
	// checkItem = tempbuildQueue.getItem();
	//
	// if(checkItem.metaType.isUnit() && checkItem.metaType.getUnitType() ==
	...
	//
	// if(MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Factory) >=
	// 1 && MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Vulture)
	// >= 1){
	//
	// bwapi.Position checker =
	// InformationManager.Instance().getFirstChokePoint(InformationManager.Instance().selfPlayer).getPoint();
	// List<Unit> eniemies = MapGrid.Instance().getUnitsNear(checker, 500, false,
	// true, null);
	//
	// for(Unit enemy : eniemies){
	//
	...
	// }
	// }
	// }else if(CurrentStrategyException ==
	// StrategysException.protossException_DragoonPush){
	//
	// }else if(CurrentStrategyException ==
	// StrategysException.protossException_ZealotPush){
	//
	// }else if(CurrentStrategyException ==
	// StrategysException.protossException_ReadyToZealot &&
	// InformationManager.Instance().getNumUnits(UnitType.Protoss_Zealot,
	...
	// }
	// else if((CurrentStrategyException ==
	// StrategysException.protossException_DragoonPush || CurrentStrategyException
	// == StrategysException.protossException_ReadyToDragoon)&& dragooncnt+zealotcnt
	// > 0
	// &&
	...
	//
	// }else if((CurrentStrategyException ==
	// StrategysException.protossException_ZealotPush || CurrentStrategyException ==
	// StrategysException.protossException_ReadyToZealot)&& dragooncnt+zealotcnt > 0
	// &&
	...
	// }
	// }else{
	// if(MyBotModule.Broodwar.self().completedUnitCount(UnitType.Terran_Command_Center)
	// ==2){
	// for (Unit unit : MyBotModule.Broodwar.self().getUnits())
	// {
	// if (unit.isLifted() == false && (unit.getType() == UnitType.Terran_Barracks
