...
public class MechanicMicroVulture extends MechanicMicroAbstract {
	...
	public void prepareMechanicAdditional(List<Unit> vultureList, List<Unit> tankList, List<Unit> goliathList,
			int saveUnitLevel, boolean attackWithMechanics) {
	...
	public void executeMechanicMicro(Unit vulture) {
		...
		MechanicMicroDecision decision = MechanicMicroDecision.makeDecision(vulture, enemiesInfo, null, saveUnitLevel); // 0:
																														// flee,
																														// 1:
																														// kiting,
																														// 2:
		...
		case 0: // flee 아예 싸울 생각이 없는 도망 : 성큰, 캐논, 시즈 등 접근금지, 또는 regroup 시
			if (order.getType() == SquadOrderType.WATCH || order.getType() == SquadOrderType.CHECK) {
				BaseLocation myBase = InformationManager.Instance()
		...
		case 1: // kiting
			...
			if (haveToFight) {
				...
				if (enemy != null && enemy.getType() == UnitType.Terran_Vulture_Spider_Mine
						&& vulture.isInWeaponRange(enemy)) {
...
//			movePosition = ShortPathGuerrilla.getNextPos(vulture.getPosition(), movePosition);
		...
		if (CombatManager.Instance().getDetailStrategyFrame(CombatStrategyDetail.MINE_STRATEGY_FOR_TERRAN) > 0) {
			...
			if (enemyFirstExpansion != null) {
				...
				if (distance < MicroSet.Tank.SIEGE_MODE_MAX_RANGE
						&& MicroUtils.isSafePlace(enemyFirstExpansion.getPosition())) {
					minePosition = SpiderMineManger.Instance().positionToMine(vulture,
							enemyFirstExpansion.getPosition(), true, MicroSet.Vulture.spiderMineNumPerPosition * 2);
				}
			}

			if (minePosition == null) {
				Position enemyReadyPos = InformationManager.Instance()
						.getReadyToAttackPosition(InformationManager.Instance().enemyPlayer);

				int distance = vulture.getDistance(enemyReadyPos);
				if (distance <= MicroSet.Tank.SIEGE_MODE_MAX_RANGE) {
					minePosition = SpiderMineManger.Instance().positionToMine(vulture, vulture.getPosition(), false,
							MicroSet.Vulture.spiderMineNumPerPosition);
				}
			}

		} else {
			minePosition = SpiderMineManger.Instance().goodPositionToMine(vulture,
					MicroSet.Vulture.spiderMineNumPerGoodPosition);

			if (minePosition == null && order.getType() == SquadOrderType.WATCH) {
				// // 적 유닛에게 마인 선물하기
				// if (InformationManager.Instance().enemyRace == Race.Terran && saveUnitLevel
				// == 0) {
				// minePosition = SpiderMineManger.Instance().enemyPositionToMine(vulture,
				// enemiesInfo);
				// }
				...
				if (mineCount <= MicroSet.Vulture.MINE_MAX_NUM) {
					...
					BaseLocation base = InformationManager.Instance()
							.getMainBaseLocation(InformationManager.Instance().selfPlayer);
					List<BaseLocation> occupiedBases = InformationManager.Instance()
							.getOccupiedBaseLocations(InformationManager.Instance().selfPlayer);

					int minePrepareLevel = SpiderMineManger.Instance().getMineInMyBaseLevel(); // 0: 본진매설X, 점령지역조금, 1:
																								// 본진매설X, 점령지역많이, 2:
																								// 본진조금, 점령지역많이
					boolean vultureInMyBaseRegion = vultureRegion == BWTA.getRegion(base.getPosition());
					if (!vultureInMyBaseRegion || minePrepareLevel >= 2) { // 본진 region에는 마인 설치안함(단 패스트 다크, 패스트 럴커 등인 경우
																			// 매설)
						boolean occupiedRegion = false;
						for (BaseLocation occupiedBase : occupiedBases) { // 앞마당 포함한 점령지역에 마인을 적게 매설함(단, 히드라웨이브, 드라군 푸시인
																			// 경우 많이 매설)
						...
						if (!occupiedRegion || (!vultureInMyBaseRegion && minePrepareLevel >= 1)) {
							minePosition = SpiderMineManger.Instance().positionToMine(vulture, vulture.getPosition(),
									false, MicroSet.Vulture.spiderMineNumPerPosition); // 그외에는 좀 많이
						} else {
							minePosition = SpiderMineManger.Instance().positionToMine(vulture, vulture.getPosition(),
									false, MicroSet.Vulture.spiderMineNumPerGoodPosition);
						}
					}
				}
			}
